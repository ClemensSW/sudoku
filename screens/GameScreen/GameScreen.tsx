...
import { View, Alert, SafeAreaView, ScrollView, useWindowDimensions } from 'react-native';
...
const GameScreen: React.FC<GameScreenProps> = ({ navigation }) => {
  ...
  const handleNumberPress = (number: number) => {
    ...
    if (noteModeActive) {
      ...
      if (updatedBoard[row][col].value === number && previousValue !== number) {
        const boardWithUpdatedNotes = removeNoteFromRelatedCells(updatedBoard, row, col, number);
  ...
  const handleHintPress = () => {
    ...
    if (board[row][col].isInitial) {
      Alert.alert(
        'Hinweis',
        'Diese Zelle ist bereits korrekt ausgefüllt.',
        [{ text: 'OK' }]
      );
      return;
    }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <Header
        title="Sudoku"
        subtitle={`Schwierigkeit: ${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}`}
        rightAction={{
          icon: "settings",
          onPress: () => {
